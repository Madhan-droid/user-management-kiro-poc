"""
User role removal Lambda handler.

This handler implements the API entry point for removing roles from users.
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for role removal.
    
    This handler processes DELETE /users/{userId}/roles/{role} requests to remove roles from users.
    
    Request flow:
    1. Create structured logger with correlation ID
    2. Log request start
    3. Extract userId and role from path parameters
    ...
    logger = create_logger(event, operation='users-role-remove')
    
    # Log request start (Requirement 12.1)
    logger.log_request_start(
        path=event.get('path', '/users/{userId}/roles/{role}'),
        method=event.get('httpMethod', 'DELETE')
    )
    
    try:
        # Extract userId and role from path parameters (Requirement 4.2)
        path_parameters = event.get('pathParameters', {})
        if not path_parameters:
            # Log validation failure (Requirement 12.2)
            logger.log_validation_error(
                errors={'path': 'userId and role are required in path'}
            )
            ...
            return create_error_response(
                ...
                {'path': 'userId and role are required in path'}
            )
        
        # Validate userId is present
        if 'userId' not in path_parameters:
            # Log validation failure (Requirement 12.2)
            logger.log_validation_error(
        ...
        if 'role' not in path_parameters:
            # Log validation failure (Requirement 12.2)
            logger.log_validation_error(
                errors={'role': 'role is required in path'}
            )
            
            return create_error_response(
                400,
                'VALIDATION_ERROR',
                'Missing role in path parameters',
                {'role': 'role is required in path'}
            )
        
        user_id = path_parameters['userId']
        role = path_parameters['role']
        ...
        # Delegate to service layer (Requirements 4.2, 4.3, 4.4)
        result = user_service.remove_role(
            user_id,
            role,
            logger.correlation_id
        )
        
        # Log successful role removal with latency (Requirement 12.2)
        ...
        # Return success response (Requirement 4.2)
