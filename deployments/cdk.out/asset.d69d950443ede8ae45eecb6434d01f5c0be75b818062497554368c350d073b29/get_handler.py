"""
User profile retrieval Lambda handler.

This handler implements the API entry point for retrieving user profiles.
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for user profile retrieval.
    
    This handler processes GET /users/{userId} requests to retrieve user profiles.
    ...
    logger = create_logger(event, operation='users-profile-get')
    ...
    try:
        # Extract userId from path parameters (Requirement 5.1)
        ...
        # Delegate to service layer (Requirements 5.1, 5.2, 5.3)
        user = user_service.get_user_by_id(user_id)
        
        # Log successful retrieval with latency (Requirement 12.2)
        ...
        return create_success_response(200, user)
